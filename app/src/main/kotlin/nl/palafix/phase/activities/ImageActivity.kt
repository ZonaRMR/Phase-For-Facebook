...
import nl.palafix.phase.facebook.FB_IMAGE_ID_MATCHER
...
class ImageActivity : KauBaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        photo.setOnImageEventListener(object : SubsamplingScaleImageView.DefaultOnImageEventListener() {
            override fun onImageLoadError(e: Exception?) {
                errorRef = e
                e.logPhaseAnswers("Image load error")
        ...
        doAsync({
            ...
            loadImage { file ->
                ...
                uiThread {
                    photo.setImage(ImageSource.uri(phaseUriFromFile(file)))
    ...
    private fun createPublicMediaFile(): File {
        ...
        val phaseDir = File(storageDir, IMG_TAG)
        if (!phaseDir.exists()) phaseDir.mkdirs()
        return File.createTempFile(imageFileName, IMG_EXTENSION, phaseDir)
    }
...
internal enum class FabStates(val iicon: IIcon, val iconColor: Int = Prefs.iconColor, val backgroundTint: Int = Int.MAX_VALUE) {
        ...
        override fun onClick(activity: ImageActivity) {
            activity.materialDialogThemed {
                ...
                onPositive { _, _ ->
                    ...
                    activity.sendPhaseEmail(R.string.debug_image_link_subject) {
    ...
    SHARE(GoogleMaterial.Icon.gmd_share) {
        override fun onClick(activity: ImageActivity) {
            try {
                val photoURI = activity.phaseUriFromFile(activity.savedFile!!)
                ...
                activity.errorRef = e
                e.logPhaseAnswers("Image share failed")
                activity.phaseSnackbar(R.string.image_share_failed)
            }
        }
    };
