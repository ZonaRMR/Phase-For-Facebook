 ...
 *
 */
object MessageParser : PhaseParser<PhaseMessages> by MessageParserImpl() {

    fun queryUser(cookie: String?, name: String) = parseFromUrl(cookie, "${FbItem.MESSAGES.url}/?q=$name")

}

data class PhaseMessages(val threads: List<PhaseThread>,
                         val seeMore: PhaseLink?,
                         val extraLinks: List<PhaseLink>
) : ParseNotification {
    override fun toString() = StringBuilder().apply {
        append("PhaseMessages {\n")
    ...
    override fun getUnreadNotifications(data: CookieModel) =
            threads.filter(PhaseThread::unread).map {
                with(it) {
                    NotificationContent(
                            data = data,
                            id = id,
 ...
 */
data class PhaseThread(val id: Long,
                       val img: String?,
                       val title: String,
                       val time: Long,
                       val url: String,
...
private class MessageParserImpl : PhaseParserBase<PhaseMessages>(true) {
    ...
    override fun parseImpl(doc: Document): PhaseMessages? {
        val threadList = doc.getElementById("threadlist_rows") ?: return null
        val threads: List<PhaseThread> = threadList.getElementsByAttributeValueContaining("id", "thread_fbid_")
        ...
        return PhaseMessages(threads, seeMore, extraLinks)
    }

    private fun parseMessage(element: Element): PhaseThread? {
