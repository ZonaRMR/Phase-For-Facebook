...
import nl.palafix.phase.rx.RxFlyweight
 ...
 */
class PhaseRequest<out T : Any?>(val call: Call, private val invoke: (Call) -> T) {
    fun invoke() = invoke(call)
}

internal inline fun <T : Any?> RequestAuth.phaseRequest(
    ...
    request.builder()
    return PhaseRequest(request.call(), invoke)
}
